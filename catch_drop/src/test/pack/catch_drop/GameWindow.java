...
public class GameWindow extends JFrame {
    ...
    private static void onRepaint(Graphics g) {
        ...
        if (drop_top > game_window.getHeight()) {
            g.drawImage(gameOver, 280, 120, null); // когда капля вылетает за границу окна, показать гейм овер


            GameField game_f = new GameField();
            game_f.addMouseListener(new MouseAdapter() {
                @Override
                public void mousePressed(MouseEvent e) { // mousePressed вызывается тогда. когда произойдет нажатие мыши
                    int x = e.getX(); // получаем координаты нажатия мыши
                    int y = e.getY();
                    if ((x <= 0 | x >= 280) && (y <=0 | y >=120)) {
